...
def test_run_factor_selection_single_fund_returns_ranked_selection(monkeypatch, sample_fund_df, tmp_path):
    ...
    preprocess_csv_path = tmp_path / "fake_preprocess.parquet"
    pd.DataFrame({
        "date": sample_index,
    ...
    }).to_parquet(preprocess_csv_path, index=False)
    ...
    metadata_path.write_text(
        json.dumps({
            "feature_preprocess_output": {
                "fund_code": "007301",
                "feature_path": str(preprocess_csv_path.resolve()),
                ...
                "factor_binding_record_list": [
                    ...
                    {
                        "output_column": "momentum(window=15)",
                        "factor_name": "momentum",
    ...
    assert result["summary_df"].iloc[0]["factor_name"] == "momentum"
    assert result["summary_df"].iloc[0]["factor_group"] == "feature_preprocess_output"
    assert result["summary_df"].iloc[0]["factor_param_dict"] == {
        "binding_mode": "single_field",
        "bound_field_name_list": ["price"],
        "bound_source_column_list": ["007301__price"],
    }
    assert result["summary_df"].iloc[0]["binding_mode"] == "single_field"
    assert result["summary_df"].iloc[0]["bound_field_name_list"] == ["price"]
    assert result["summary_df"].iloc[0]["bound_source_column_list"] == ["007301__price"]
    ...
    assert saved_payload["factor_selection_output"]["record_dict"]["momentum(window=10)"]["factor_name"] == "momentum"
    assert saved_payload["factor_selection_output"]["record_dict"]["momentum(window=10)"]["binding_mode"] == "single_field"
    assert saved_payload["factor_selection_output"]["record_dict"]["momentum(window=10)"]["bound_field_name_list"] == ["price"]
    assert saved_payload["factor_selection_output"]["record_dict"]["momentum(window=10)"]["bound_source_column_list"] == ["007301__price"]

def test_run_factor_selection_single_fund_supports_feature_path_from_metadata(monkeypatch, tmp_path):
    feature_csv_path = tmp_path / "feature_preprocess_007301_2026-04-12_abcd0.parquet"
    ...
    feature_df.to_parquet(feature_csv_path, index=False)
        ...
        json.dumps(
            {
                ...
                "feature_preprocess_output": {
                    "fund_code": "007301",
                    "feature_path": str(feature_csv_path.resolve()),
                    "feature_format": "parquet",
...
def test_run_factor_selection_single_fund_allows_metadata_fund_code_to_differ_from_default_config(monkeypatch, tmp_path):
    feature_path = tmp_path / "feature_preprocess_008115_2026-04-12_abcd0.parquet"
    metadata_json_path = tmp_path / "feature_preprocess_008115_2026-04-12_abcd0.json"
    sample_index = pd.date_range("2024-01-01", periods=30, freq="D")
    feature_df = pd.DataFrame(
        {
            "date": sample_index,
            "008115__price": [1.0 + idx * 0.01 for idx in range(len(sample_index))],
            "008115__cumulative_nav": [1.0 + idx * 0.01 for idx in range(len(sample_index))],
            "momentum(window=10)": list(range(len(sample_index))),
        }
    )
        ...
        json.dumps(
            {
                ...
                "feature_preprocess_output": {
                    ...
                    "feature_format": "parquet",
                    "target_nav_column": "008115__cumulative_nav",
                    "target_price_column": "008115__price",
                    "factor_feature_column_list": ["momentum(window=10)"],
                    "factor_binding_record_list": [
                        {
                            "output_column": "momentum(window=10)",
                            "factor_name": "momentum",
                            ...
                            "binding_mode": "single_field",
                            "bound_field_name_list": ["price"],
                            "bound_source_column_list": ["008115__price"],
                        },
                    ],
                },
            },
    ...
    monkeypatch.setattr(
        ...
        lambda factor_series, forward_return_series, segment_index: {
            pd.Timestamp("2024-01-01"): {"sample_size": len(segment_index), "spearman_ic": 0.4, "pearson_ic": 0.3},
            pd.Timestamp("2024-01-06"): {"sample_size": len(segment_index), "spearman_ic": 0.5, "pearson_ic": 0.35},
            pd.Timestamp("2024-01-11"): {"sample_size": len(segment_index), "spearman_ic": 0.4, "pearson_ic": 0.3},
            pd.Timestamp("2024-01-16"): {"sample_size": len(segment_index), "spearman_ic": 0.5, "pearson_ic": 0.35},
        }[pd.Index(segment_index).min()],
    )

    result = factor_analysis.run_factor_selection_single_fund()

    assert result["fund_code"] == "008115"
    assert result["selected_factor_name_list"] == ["momentum"]
    assert result["selected_candidate_label_list"] == ["momentum(window=10)"]
